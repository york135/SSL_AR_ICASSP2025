...
# https://github.com/0nutation/SpeechGPT/blob/main/speechgpt/utils/speech2unit/speech2unit.py
...
def get_mhubert_discrete_feature(s2u, audio_path_list):
    all_features = {}
    with torch.no_grad():
        for audio_path in tqdm(audio_path_list):
            cur_features = torch.zeros((1, 1000))     
            try:
                units = s2u(audio_path)
            except:
                continue   

            for i in range(len(units)):
                cur_features[0][units[i]] += 1
