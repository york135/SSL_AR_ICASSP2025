...
def extract_wavlm_large_cont():
    ...
    audio_path_list = []
    with open('VCTK-Corpus-0.92/VCTK_dataset_partitions_2023.csv', newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            if row[0] == 'File':
                continue
            audio_path_list.append(os.path.join('VCTK-Corpus-0.92/wav48_silence_trimmed', row[0].split('.')[0] + '.flac'))
                
    print (len(audio_path_list), audio_path_list[0])

    wavlm_features = get_cont_feature(model, audio_path_list)
    wavlm_pkl_path = 'vctk_wavlm_large_cont.pkl'
...
def extract_wavlm_large_disc():
    ...
    for kmeans_filename in kmeans_filename_list:
        ...
        wavlm_pkl_path = 'vctk_wavlm_large_disc_L' + str(ssl_layer_num) + '.pkl'
        with open(wavlm_pkl_path, 'wb') as f:
            pickle.dump(wavlm_features, f)
...
def extract_mhubert_cont():
    ...
    wavlm_features = get_cont_feature(model, audio_path_list)
    wavlm_pkl_path = 'vctk_mhubert_cont.pkl'
    with open(wavlm_pkl_path, 'wb') as f:
        pickle.dump(wavlm_features, f)
...
def extract_mhubert_disc():
    ...
    wavlm_pkl_path = 'vctk_mhubert_disc_L' + str(ssl_layer_num) + '.pkl'
    with open(wavlm_pkl_path, 'wb') as f:
        pickle.dump(wavlm_features, f)
